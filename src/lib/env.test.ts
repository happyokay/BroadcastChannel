...
describe('getEnv', () => {
  ...
  it('accepts the legacy Astro context argument', () => {
    expect(
      getEnv(
        { TEST_ENV_PRIORITY: 'import-value' },
        {},
...
describe('getBooleanEnv', () => {
  ...
  it('accepts the legacy Astro context argument', () => {
    vi.stubEnv('TEST_BOOLEAN_ENV', '1')

    expect(getBooleanEnv({}, {}, 'TEST_BOOLEAN_ENV')).toBe(true)
  })
})
...
describe('getPodcastUrl', () => {
  ...
  it('reads the podcast URL for the existing header UI', () => {
    vi.stubEnv('PODCAST', 'https://example.com/podcast.xml')

    expect(getPodcastUrl({}, {})).toBe('https://example.com/podcast.xml')
  })
})
