...
function getImages($: CheerioAPI, message: MessageSelection, options: MessageAssetOptions): string {
  ...
  for (const [photoIndex, photoNode] of message.find('.tgme_widget_message_photo_wrap').toArray().entries()) {
    ...
    fragments.push(`
      ...
      >
        <img src="${getProxiedUrl(staticProxy, imageUrl)}" alt="${safeTitle}" width="${width}" height="${height}" loading="${loading}" decoding="async" style="--image-natural-width: ${width}px; --image-natural-height: ${height}px;" />
      </button>
      <div class="modal" id="${popoverId}" popover aria-label="Image preview">
        ...
        <div class="modal__surface">
          <img class="modal-img" src="${getProxiedUrl(staticProxy, imageUrl)}" alt="${safeTitle}" width="${width}" height="${height}" loading="lazy" decoding="async" style="--image-natural-width: ${width}px; --image-natural-height: ${height}px;" />
        </div>
      </div>
    `)
  }
